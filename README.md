...
**Blocking patterns** (PreToolUse, in settings.json): Three hooks block destructive actions by default. `block-rm-rf.py` blocks `rm -rf` (points at `rm -r` or `find <path> -delete` on a specific path) and `block-push-main.py` blocks direct push to main/master (requires feature branches). `safety-block.py` covers a wider set -- `dd`/`mkfs`/`wipefs` against a disk, fork bombs, `chmod -R 777` of `/` or `~`, disk-partitioning tools, and force-pushes to protected branches. All three share the `cmdscan` tokenizer, so nested `bash -c` payloads and unspaced separators are unwrapped and checked. All three exit 2 with an error message that tells Claude what to do instead, and all are invoked via `run-hook.sh <name>` rather than directly. See `hooks/block-rm-rf.py`, `hooks/block-push-main.py`, and `hooks/safety-block.py`.
