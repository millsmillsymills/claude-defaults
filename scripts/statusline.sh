...
if [ -n "$ctx_used" ] && [ "$ctx_used" != "null" ]; then
    ...
    for ((i=0; i<filled; i++)); do
        progress_bar="${progress_bar}\xe2\x96\x88"
    done
    progress_bar="${progress_bar}\033[2m"
    for ((i=0; i<empty; i++)); do
        progress_bar="${progress_bar}\xe2\xa3\xbf"
    done
...
line1="$line1 $(printf '\033[94m\xf0\x9f\x93\x81 %s\033[0m' "$folder_name")"
if [ -n "$git_branch" ]; then
    line1="$line1 $(printf '%b \033[96m\xf0\x9f\x8c\xbf %s\033[0m' "$SEP" "$git_branch")"
fi
...
if [ -n "$session_time" ]; then
    line2="$line2 $(printf '%b \033[36m\xe2\x8f\xb1 %s\033[0m' "$SEP" "$session_time")"
fi
if [ "$cache_pct" -gt 0 ] 2>/dev/null; then
    line2="$line2 $(printf ' \033[2m\xe2\x86\xbb%s%%\033[0m' "$cache_pct")"
fi
